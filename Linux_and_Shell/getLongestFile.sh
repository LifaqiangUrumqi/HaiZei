...
function getLongestString() {
    file=$1
    echo "Do you want to run this program?[y/n]"
    read validate
    if [ "$validate" == "y" ] || [ "$validate" == "Y" ] ||[ "$validate" == "yes" ] || [ "$validate" == "Yes" ] || [ "$validate" == "YES" ]; then #进行可靠性验证
        ...
        echo "You are welcome to use this program again."
    fi
}
